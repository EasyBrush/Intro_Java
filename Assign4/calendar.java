package Assign4;
/*This program utilizes the methods printMonthCalendar, printMonthHeader,
 * printMonthBody, getStartDay, getNumDaysinMonth, isLeapYear, and
 * getMonthName. Program prompts user to input a valid month 
 * (an integer from 1-12), and a valid year (e.g. 2012) and prints out
 * the calendar corresponding to the user's input values. 
 * 
 */
...
public class calendar 
{
	...
	public static void main(String[] args) 
	{
		...
		int month = 0;	//Variables
		int year = 0;		
		
		System.out.println("Enter a month (1-12)");	//User input 
		month = input.nextInt();
		System.out.println("Enter a year (e.g. 2012)");
		year = input.nextInt();
		
		printMonthCalendar(month, year);//Runs method to print Calendar
		input.close();
	}	
 ...
 */
	...
	{
		int i = 0;
		for( i = 0; i < (getStartDay(month,year)%7); i++) //fixes shifting problem
		{        										
 ...
 * postcondition: based on the year and month the method will return either 31, 30, 29,
 * or 28 as the number of days in that corresponding month
 */
